    ...
    _write_csv(
        ...
        [
            ...
            },
            {
                ...
                "attempted": "true",
                "succeeded": "true",
                "raw_contributor_rows_before_filter": 10,
                "unique_raw_pixel_count": 7,
                "selected_pixel_hit_count_normal": 0,
                "y_flip_hit_count": 0,
                "x_flip_hit_count": 0,
                "xy_swap_hit_count": 1,
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="viewtrust-pr211f-") as tmp:
        ...
        assert all(row["reconstructed_residual_available"] == "false" for row in residual_rows)
        assert all(row["interpretation"] == "residual_source_unavailable" for row in residual_rows)

        diagnosis = _read_csv(output / "pr211f_drums_alignment_diagnosis.csv")
        modes = {row["likely_failure_mode"] for row in diagnosis}
        assert "selected_pixel_coordinate_flip_candidate" in modes
        assert "exact_replay_has_no_raw_contributors" in modes
